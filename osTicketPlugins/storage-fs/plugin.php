...
class FsStoragePluginConfig extends PluginConfig {
    ...
    function pre_save($config, &$errors) {
        ...
        $form = $this->getForm();
        $field = $form->getField('uploadpath');
        print_r($form);
        $path = $field['_clean']['uploadpath'];
        $file = md5(microtime());
        if (!@is_dir($path))
...
class FsStoragePlugin extends Plugin {
    ...
    function bootstrap() {
        ...
        if ($uploadpath) {
            FileStorageBackend::register('FilesystemStorage');
...
/**
 * Called by FsStorage plugin.
 * @author Grizly
 *
 * Creates a new "Backend" that can be registered into the osTicket system.
 */
class FileStorageBackend extends Backend{
	public static $base;
	public static $desc;
